...
```{r}
# Read object in:
tmpFn <- "../Robjects/caron_postDeconv_5hCellPerSpl_dsi_PBMMC_ETV6-RUNX1_clust.Rds"
file.exists(tmpFn)
sce <- readRDS(tmpFn)
sce
colData(sce) %>% head
...
On the t-SNE plots below, cells coloured by type or sample ('batch of origin'). Cluster numbers are superimposed based on the median coordinate of cells assigned to that cluster. 
...
<!-- start from uncorrected from scratch
and set merge.order --> 
...
hhd_index <- grep("HHD", levels(uncorrected$SampleName)) %>% as.list
pret_index <- grep("PRE-T", levels(uncorrected$SampleName)) %>% as.list

if(splSetToGet == "PBMMC,ETV6-RUNX1") {
  #mergeOrderList <- list( as.list(etv6runx1_index), as.list(pbmmc_index) )
  mergeOrderList <- list( pbmmc_index,
                          etv6runx1_index )
...
tmpAnnot <- rowData(current)[,c("ensembl_gene_id","Symbol")] %>% data.frame # ensid can be ID
...
for (labelToGet in levels(summed$label)) {
    ...
    y <- y[,!discarded]
    y <- y[filterByExpr(y, group=current$SampleGroup),]
    y <- calcNormFactors(y)

    design <- try(
        model.matrix(~factor(SampleGroup), y$samples),
        silent=TRUE
    )
...
clustersToCheck <- setdiff(levels(merged$clusters.mnn), skippedClusters)

for( namex in clustersToCheck)
{
...
grmToShowDf %>%
	select(all_of(tmpCol), setdiff(colnames(grmToShowDf), tmpCol)) %>%
...
plotExpression(logNormCounts(summed.filt), 
    ...
    other_fields="label") + 
    facet_wrap(~label) +
  ggtitle(rownames(de.inspec[[cx]])[1])
```
